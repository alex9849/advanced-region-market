...
public class OldRegionManager {
    ...
    public static void loadRegionsFromConfig() {
        ...
        updateConfig();
        if(OldRegionManager.getRegionsConf().get("Regions") != null) {
            ConfigurationSection mainSection = OldRegionManager.getRegionsConf().getConfigurationSection("Regions");
                        ...
                        if(regions != null) {
                            for (int i = 0; i < regions.size(); i++) {
                                ...
                                if(regionsconf.get("Regions." + worlds.get(y)+ "." + regions.get(i) + ".subregions") != null) {
                                    List<String> subregions = new ArrayList<String>(regionsconf.getConfigurationSection("Regions." + worlds.get(y)+ "." + regions.get(i) + ".subregions").getKeys(false));
                                    if(subregions != null) {
    ...
    public static YamlConfiguration getRegionsConf() {
        return OldRegionManager.regionsconf;
    }

    public static void setRegionsConf(){
        File pluginfolder = Bukkit.getPluginManager().getPlugin("AdvancedRegionMarket").getDataFolder();
        File regionsconfigdic = new File(pluginfolder + "/regions.yml");
        OldRegionManager.regionsconf = YamlConfiguration.loadConfiguration(regionsconfigdic);
    }
    ...
    public static void saveRegionsConf() {
        ...
        File regionsconfigdic = new File(pluginfolder + "/regions.yml");
        try {
            OldRegionManager.regionsconf.save(regionsconfigdic);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public static List<String> completeTabRegions(Player player, String arg, PlayerRegionRelationship playerRegionRelationship, boolean inculdeNormalRegions, boolean includeSubregions) {
        ...
        if(Region.completeTabRegions) {
            ...
            for(Region region : OldRegionManager.getRegionList()) {
    ...
    public static List<Region> getRegionsByMember(UUID uuid) {
        ...
        for (int i = 0; i < OldRegionManager.getRegionList().size(); i++){
            for(Region subregion : OldRegionManager.getRegionList().get(i).getSubregions()) {
                if(subregion.getRegion().hasMember(uuid)){
                    regions.add(subregion);
                }
            }
            if(OldRegionManager.getRegionList().get(i).getRegion().hasMember(uuid)) {
                regions.add(OldRegionManager.getRegionList().get(i));
            }
        }
        return regions;
    }
    ...
    public static List<Region> getRegionsByOwner(UUID uuid) {
        ...
        for (int i = 0; i < OldRegionManager.getRegionList().size(); i++){
            ...
            if(OldRegionManager.getRegionList().get(i).getRegion().hasOwner(uuid)){
                regions.add(OldRegionManager.getRegionList().get(i));
            }
        }
        return regions;
    ...
    public static boolean autoResetRegionsFromOwner(UUID uuid){
        List<Region> regions = OldRegionManager.getRegionsByOwner(uuid);
        for(int i = 0; i < regions.size(); i++){
    ...
    public static void teleportToFreeRegion(RegionKind type, Player player) throws InputException {
        for (int i = 0; i < OldRegionManager.getRegionList().size(); i++){
            ...
            if ((OldRegionManager.getRegionList().get(i).isSold() == false) && (OldRegionManager.getRegionList().get(i).getRegionKind() == type)){
                WGRegion regionTP = OldRegionManager.getRegionList().get(i).getRegion();
                String message = Messages.REGION_TELEPORT_MESSAGE.replace("%regionid%", regionTP.getId());
                Teleporter.teleport(player, OldRegionManager.getRegionList().get(i), Messages.PREFIX + message, true);
                return;
            }
        }
    ...
    public static boolean containsRegion(Region region) {
        return OldRegionManager.regionList.contains(region);
    }
